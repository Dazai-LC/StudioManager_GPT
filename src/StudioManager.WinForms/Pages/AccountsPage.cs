...
public sealed class AccountsPage : UserControl
{
    ...
    private async Task LoadAsync(){var rows=await _app.Repository.QueryGridAsync("TaiKhoan",string.IsNullOrWhiteSpace(_search.Text)?null:_search.Text.Trim());var dt=new System.Data.DataTable();if(rows.Count>0){foreach(var k in rows[0].Keys)dt.Columns.Add(k,typeof(object));foreach(var r in rows){var x=dt.NewRow();foreach(var p in r)x[p.Key]=p.Value??DBNull.Value;dt.Rows.Add(x);}}_grid.DataSource=dt;if(_grid.Columns.Contains("Id"))_grid.Columns["Id"].Visible=false;}
    private async Task CreateAsync(){using var d=new AccountDialog(await _app.Repository.GetLookupsAsync("NhanVien"));if(d.ShowDialog(this)!=DialogResult.OK)return;var r=await _app.Repository.CreateAccountAsync(d.Username,_app.PasswordHasher.Hash(d.Password),d.EmployeeId,d.Role,_app.Session!);if(!r.Success)Ui.Error(this,r.Message);else{Ui.Info(this,r.Message);await LoadAsync();}}
    private async Task ResetAsync(){var id=Id();if(id is null)return;using var d=new PasswordPrompt();if(d.ShowDialog(this)!=DialogResult.OK)return;var r=await _app.Repository.ResetPasswordAsync(id.Value,_app.PasswordHasher.Hash(d.Value),_app.Session!);if(!r.Success)Ui.Error(this,r.Message);else Ui.Info(this,r.Message);}
    private async Task LockAsync(){var id=Id();if(id is null)return;if(id==_app.Session!.TaiKhoanId){Ui.Error(this,"Không thể khóa tài khoản đang đăng nhập.");return;}var r=await _app.Repository.DeactivateAsync("TaiKhoan",id.Value,_app.Session);if(!r.Success)Ui.Error(this,r.Message);else await LoadAsync();}
