...
internal sealed class BookingDetailDialog : Form
{
    ...
    {
        _summary.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute,180));_summary.ColumnStyles.Add(new ColumnStyle(SizeType.Percent,50));_summary.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute,184));_summary.ColumnStyles.Add(new ColumnStyle(SizeType.Percent,50));
    ...
    private async Task ReloadAsync(){_booking=await _app.Bookings.GetAsync(_booking.Id)??_booking;UpdateSummary();Bind(_services,await _app.BookingSupport.GetChildrenAsync(_booking.Id,"DichVu"));Bind(_resources,await _app.BookingSupport.GetChildrenAsync(_booking.Id,"TaiNguyen"));var thu=await _app.BookingSupport.GetChildrenAsync(_booking.Id,"ThanhToan");var hoan=await _app.BookingSupport.GetChildrenAsync(_booking.Id,"HoanTien");Bind(_payments,thu.Select(x=>TransactionRow(x,false)).Concat(hoan.Select(x=>TransactionRow(x,true))).Cast<IDictionary<string,object?>>().ToList());}
    private static Dictionary<string,object?> TransactionRow(IDictionary<string,object?> row,bool refund)
    {
        var rawType=row.TryGetValue("Loại",out var raw)&&raw is not null?raw.ToString():null;
        var type=refund?"Hoàn tiền":rawType switch{"DAT_COC"=>"Đặt cọc","BO_SUNG"=>"Thu bổ sung","CON_LAI"=>"Thanh toán còn lại",_=>rawType??"Thanh toán"};
        var note=refund&&row.TryGetValue("Lý do",out var reason)?reason:row.TryGetValue("Ghi chú",out var remark)?remark:null;
        return new(){["Id"]=row.TryGetValue("Id",out var id)?id:null,["Mã"]=row.TryGetValue("Mã",out var code)?code:null,["Loại"]=type,["Số tiền"]=row.TryGetValue("Số tiền",out var amount)?amount:null,["Ngày"]=row.TryGetValue("Ngày",out var date)?date:null,["Ghi chú"]=note};
    }
