...
public sealed class CrudPage : UserControl
{
    ...
    public CrudPage(AppFacade app,string title,string entity,bool readOnly=false)
    {
        ...
        var toolbar=new FlowLayoutPanel{Dock=DockStyle.Top,AutoSize=true,AutoSizeMode=AutoSizeMode.GrowAndShrink,WrapContents=true,Padding=new Padding(0,7,0,7)};toolbar.Controls.Add(_search);var find=Theme.Button("Tìm kiếm",Color.FromArgb(71,85,105));find.Click+=async(_,_)=>await LoadAsync();toolbar.Controls.Add(find);var refresh=Theme.Button("↻ Làm mới",Theme.Cyan);refresh.Click+=async(_,_)=>{_search.Clear();await LoadAsync();};toolbar.Controls.Add(refresh);var details=Theme.Button("Xem chi tiết",Theme.Purple);details.Click+=(_,_)=>ShowDetails();toolbar.Controls.Add(details);var export=Theme.Button("Xuất CSV",Theme.Success);export.Click+=(_,_)=>Ui.ExportGrid(_grid,this,_entity);toolbar.Controls.Add(export);var edit=Theme.Button("Chỉnh sửa",Theme.Primary);edit.Visible=!readOnly;edit.Click+=async(_,_)=>await EditAsync(SelectedId());toolbar.Controls.Add(edit);var stop=Theme.Button(entity=="KhachHang"?"Xóa":"Ngừng dùng",Theme.Danger);stop.Visible=!readOnly;stop.Click+=async(_,_)=>await DeactivateAsync();toolbar.Controls.Add(stop);
    ...
    private async Task LoadAsync(){try{var rows=await _app.Repository.QueryGridAsync(_entity,string.IsNullOrWhiteSpace(_search.Text)?null:_search.Text.Trim());var table=new BindingList<RowView>(rows.Select(x=>new RowView(x)).ToList());_grid.DataSource=table;if(_grid.Columns.Contains("Data"))_grid.Columns["Data"].Visible=false;BuildColumns(rows);}catch(Exception ex){Ui.Error(this,ex.Message);}}
    ...
    private async Task EditAsync(long? id){if(_readOnly)return;using var dialog=new SimpleEditDialog(_entity,id,_grid.CurrentRow);if(dialog.ShowDialog(this)!=DialogResult.OK)return;var result=await _app.Repository.SaveSimpleAsync(_entity,id,dialog.Values,_app.Session!);if(!result.Success)Ui.Error(this,result.Message);else{Ui.Info(this,result.Message);await LoadAsync();}}
    private async Task DeactivateAsync(){var id=SelectedId();if(id is null)return;var message=_entity=="KhachHang"?"Xóa khách hàng đã chọn? Chỉ khách hàng chưa có lịch mới được xóa.":"Ngừng sử dụng mục đã chọn? Dữ liệu lịch sử vẫn được giữ nguyên.";if(MessageBox.Show(message,"Xác nhận",MessageBoxButtons.YesNo,MessageBoxIcon.Question)!=DialogResult.Yes)return;var r=await _app.Repository.DeactivateAsync(_entity,id.Value,_app.Session!);if(!r.Success)Ui.Error(this,r.Message);else await LoadAsync();}
