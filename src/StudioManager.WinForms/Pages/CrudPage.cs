...
public sealed class CrudPage : UserControl
{
    ...
    private void BuildColumns(IReadOnlyList<IDictionary<string,object?>> rows){if(rows.Count==0)return;_grid.DataSource=null;var dt=new System.Data.DataTable();foreach(var key in rows[0].Keys)dt.Columns.Add(key,typeof(object));foreach(var r in rows){var row=dt.NewRow();foreach(var p in r)row[p.Key]=p.Value??DBNull.Value;dt.Rows.Add(row);}_grid.DataSource=dt;if(_grid.Columns.Contains("Id"))_grid.Columns["Id"].Visible=false;if(_entity=="TaiNguyen"&&_grid.Columns.Contains("Dịch vụ thuê ID"))_grid.Columns["Dịch vụ thuê ID"].Visible=false;}
    private async Task EditAsync(long? id){if(_readOnly)return;IReadOnlyList<LookupItem>? rentalServices=null;if(_entity=="TaiNguyen")rentalServices=await _app.Administration.GetLookupsAsync("DichVu",id is null);using var dialog=new SimpleEditDialog(_entity,id,_grid.CurrentRow,rentalServices);if(dialog.ShowDialog(this)!=DialogResult.OK)return;var result=await _app.Administration.SaveAsync(_entity,id,dialog.Values,_app.Session!);if(!result.Success)Ui.Error(this,result.Message);else{Ui.Info(this,result.Message);await LoadAsync();}}
    ...
    private async Task ShowHistoryAsync(){var id=SelectedId();if(id is null){Ui.Error(this,"Vui lòng chọn một dòng dữ liệu.");return;}try{var rows=await _app.Administration.GetBookingHistoryAsync(_entity,id.Value,_app.Session!);using var dialog=new BookingHistoryDialog(_entity,rows);dialog.ShowDialog(this);}catch(Exception ex){Ui.Error(this,ex.Message);}}
...
internal sealed class SimpleEditDialog : Form
{
    ...
    private static IEnumerable<FieldDef> Fields(string e)=>e switch
    {
        ...
        "TaiNguyen"=>[new("TenTaiNguyen","Tên tài nguyên *"),new("LoaiTaiNguyen","Loại",["THIET_BI","TRANG_PHUC"]),new("TongSoLuong","Tổng số lượng *",null,true),new("DichVuThueId","Dịch vụ thuê liên kết"),new("GhiChu","Ghi chú"),new("TrangThai","Trạng thái",["HOAT_DONG","NGUNG_SU_DUNG"])],_=>[]
    };
    private static Control CreateInput(FieldDef f,IReadOnlyList<LookupItem>? rentalServices){if(f.Name=="DichVuThueId"){var c=new ComboBox{DropDownStyle=ComboBoxStyle.DropDownList,DisplayMember="Name",ValueMember="Id"};var items=new List<LookupItem>{new(0,"","Không liên kết")};if(rentalServices is not null)items.AddRange(rentalServices);c.DataSource=items;return c;}if(f.Options is not null){var c=new ComboBox{DropDownStyle=ComboBoxStyle.DropDownList};c.Items.AddRange(f.Options);if(c.Items.Count>0)c.SelectedIndex=0;return c;}if(f.Number)return new NumericUpDown{Maximum=1_000_000_000,ThousandsSeparator=true};return new TextBox();}
    private void Fill(DataGridViewRow row){var grid=row.DataGridView;if(grid is null)return;foreach(var p in _inputs){var col=grid.Columns.Cast<DataGridViewColumn>().FirstOrDefault(c=>Normalize(c.HeaderText).Contains(Normalize(LabelFor(p.Key)))||Normalize(c.Name).Contains(Normalize(LabelFor(p.Key))));if(col is null)continue;var value=row.Cells[col.Index].Value;if(p.Key=="DichVuThueId"&&p.Value is ComboBox rental&&long.TryParse(value?.ToString(),out var rentalId)){rental.SelectedValue=rentalId;continue;}if(p.Value is ComboBox cb)cb.SelectedItem=value?.ToString();else if(p.Value is NumericUpDown n&&decimal.TryParse(value?.ToString(),out var d))n.Value=Math.Min(n.Maximum,d);else p.Value.Text=value?.ToString()??"";}}
    ...
    private static object? Read(Control c)=>c switch{NumericUpDown n=>n.Value,ComboBox { SelectedItem: LookupItem item }=>item.Id==0?null:item.Id,ComboBox cb=>cb.SelectedItem?.ToString(),_=>string.IsNullOrWhiteSpace(c.Text)?null:c.Text.Trim()};
    private static string Normalize(string s)=>new string(s.Where(char.IsLetterOrDigit).ToArray()).ToLowerInvariant();private static string LabelFor(string k)=>k switch{"HoTen"=>"Họ tên","SoDienThoai"=>"Điện thoại","TenGoiChup"=>"Tên gói","GiaGoi"=>"Giá","ThoiLuongPhut"=>"Thời lượng","TenDichVu"=>"Tên dịch vụ","DonViTinh"=>"Đơn vị","DonGia"=>"Đơn giá","TenPhong"=>"Tên phòng","TenTaiNguyen"=>"Tên tài nguyên","LoaiTaiNguyen"=>"Loại","TongSoLuong"=>"Số lượng","DichVuThueId"=>"Dịch vụ thuê ID","GhiChu"=>"Ghi chú","MoTa"=>"Mô tả","TrangThai"=>"Trạng thái",_=>k};
    private sealed record FieldDef(string Name,string Label,string[]? Options=null,bool Number=false);
}

internal sealed class BookingHistoryDialog : Form
{
    public BookingHistoryDialog(string entity,IReadOnlyList<IDictionary<string,object?>> rows)
    {
        Text=$"Lịch sử lịch chụp • {entity}";StartPosition=FormStartPosition.CenterParent;ClientSize=new(960,540);MinimumSize=new(760,420);BackColor=Theme.Background;Font=Theme.Font();
        var grid=Theme.Grid();var table=new System.Data.DataTable();if(rows.Count>0){foreach(var key in rows[0].Keys)table.Columns.Add(key,typeof(object));foreach(var source in rows){var row=table.NewRow();foreach(var value in source)row[value.Key]=value.Value??DBNull.Value;table.Rows.Add(row);}}grid.DataSource=table;if(grid.Columns.Contains("Id"))grid.Columns["Id"].Visible=false;
        var empty=new Label{Dock=DockStyle.Top,Height=42,Text=rows.Count==0?"Chưa có lịch chụp liên quan.":$"Có {rows.Count} lịch chụp liên quan.",TextAlign=ContentAlignment.MiddleLeft,ForeColor=Theme.Muted,Padding=new Padding(18,0,18,0)};
        Controls.Add(grid);Controls.Add(empty);
    }
}
